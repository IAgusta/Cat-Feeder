...
controlData controlPacket; //instance of the data structure (send)

struct receivedData {
  int afterRun;
};
receivedData receivedPacket; //instance of the data structure (received)
...
void loop() {
  ...
  buttonState = digitalRead(buttonBeri);
  if(buttonState == HIGH){
    controlPacket.servoRun = 1;
    Serial.println("Button Pressed, Operating Food Feeder");
    controlPacket.weight = 50;
    Serial.println("Automatically Input Weight 50g");
    dataChange = true;
  }

  //if data got changed it will send to slave
  if (dataChange) {
    esp.write((uint8_t*)&controlPacket, sizeof(controlPacket));
    Serial.println("Control packet sent to Slave ESP");
    //set status on blynk and lcd
    Blynk.virtualWrite(V3,"Proses Pemberian Pakan");
    lcd.setCursor(0,0);
    lcd.print(" ");
    lcd.setCursor(0,0);
    String berat = String(controlPacket.weight) + "g";
    lcd.print("Beri Pakan :" + berat);
    delay(500);
    clearLCD(0);
  }
  ...
  if (esp.available() > sizeof(receivedData)) {
    esp.readBytes((uint8_t*) &receivedPacket, sizeof(receivedData));
    int servoRunInt = receivedPacket.afterRun ? 1 : 0;
    // Update your control packet with received servoRun data
    controlPacket.servoRun = receivedPacket.afterRun;
